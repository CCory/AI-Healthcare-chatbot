      ...
      <Layout style={{ minHeight: '100vh' }}>
        <Sider collapsible collapsed={collapsed} onCollapse={onCollapse}>
          ...
          >
            ...
            <SubMenu key="sub1" icon={<UserOutlined />} title="Medici">
              {doctors &&
                doctors.map((doctorName) => (
                  <Menu.Item key={doctorName}>{doctorName}</Menu.Item>
                ))}
            </SubMenu>
            <SubMenu key="sub2" icon={<TeamOutlined />} title="Specializari">
        ...
        <Layout className="site-layout">
          <Header className="site-layout-background cp__container__header">
            <h2 className="cp__header">Control Panel </h2>
            <p className="cp__p"> Bine ai revenit {name}</p>
